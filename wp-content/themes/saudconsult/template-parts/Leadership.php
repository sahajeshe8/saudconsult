<?php
/**
 * Leadership Section Component Template
 *
 * @package tasheel
 */

?>

<section class="leadership_section pt_80">
	<div class="wrap">

<div class="leadership_section_inner_content pb_50">

 

<ul class="leadership_section_inner_left_list"> 
 
<li data-aos="fade-up" data-aos-duration="800" data-aos-delay="0">
<span class="lable_text green_text mb_50">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/images/dot-02.svg" alt="Our Projects">
					Our Projects
				</span>

				<h3 class="h3_title_50">
					 
                Our Visionary <span>Leadership</span>
				</h3>
</li>
<li data-aos="fade-up" data-aos-duration="800" data-aos-delay="100">
    <div class="leadership_block">
       <div class="leadership_img">
        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/l1.jpg" alt="Leadership Image">
       </div>
       <div class="leadership_content">
        <h4>Sami Sidawi</h4>
        <div class="leadership_content_bottom">
            <p> <img src="<?php echo get_template_directory_uri(); ?>/assets/images/green-dot.svg" alt="Quote"> Chairman</p>

            <a href="#"  >
                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/in.svg" alt="Arrow Right">
            </a>
        </div>
      
       </div>
    </diuv>
</li>
<li data-aos="fade-up" data-aos-duration="800" data-aos-delay="200">
    <div class="leadership_block">
       <div class="leadership_img">
        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/l2.jpg" alt="Leadership Image">
       </div>
       <div class="leadership_content">
        <h4>John N Helou</h4>
        <div class="leadership_content_bottom">
            <p> <img src="<?php echo get_template_directory_uri(); ?>/assets/images/green-dot.svg" alt="Quote"> Chairman</p>

            <a href="#"  >
                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/in.svg" alt="Arrow Right">
            </a>
        </div>
      
       </div>
    </diuv>
</li>
</ul>

            






	 

		 
</div>







<div class="leadership_section_inner_right  " data-aos="fade-up" data-aos-duration="800" data-aos-delay="300">
  
    <h4 class="h4_title_35 pb_20">
    Our Executive <span>Team</span>
				</h4>




                <ul class="leadership_section_inner_left_list ">
                <li data-aos="fade-up" data-aos-duration="800" data-aos-delay="400">
     <div class="leadership_block">
        <div class="leadership_img">
         <img src="<?php echo get_template_directory_uri(); ?>/assets/images/l3.jpg" alt="Leadership Image">
        </div>
        <div class="leadership_content">
         <h4>Hadi Sha</h4>
         <div class="leadership_content_bottom">
             <p> <img src="<?php echo get_template_directory_uri(); ?>/assets/images/green-dot.svg" alt="Quote"> Chief Development Officer</p>
 
             <a href="#"  >
                 <img src="<?php echo get_template_directory_uri(); ?>/assets/images/in.svg" alt="Arrow Right">
             </a>
         </div>
       
        </div>
     </diuv>
 </li>
 <li data-aos="fade-up" data-aos-duration="800" data-aos-delay="500">
     <div class="leadership_block">
        <div class="leadership_img">
         <img src="<?php echo get_template_directory_uri(); ?>/assets/images/l4.jpg" alt="Leadership Image">
        </div>
        <div class="leadership_content">
         <h4>Mohammed El D</h4>
         <div class="leadership_content_bottom">
             <p> <img src="<?php echo get_template_directory_uri(); ?>/assets/images/green-dot.svg" alt="Quote">Chief Operating Officer</p>
 
             <a href="#"  >
                 <img src="<?php echo get_template_directory_uri(); ?>/assets/images/in.svg" alt="Arrow Right">
             </a>
         </div>
       
        
     </div>
 </li>
 <li data-aos="fade-up" data-aos-duration="800" data-aos-delay="600">
     <div class="leadership_block">
        <div class="leadership_img">
         <img src="<?php echo get_template_directory_uri(); ?>/assets/images/l5.jpg" alt="Leadership Image">
        </div>
        <div class="leadership_content">
         <h4>Jihad Khoury</h4>
         <div class="leadership_content_bottom">
             <p> <img src="<?php echo get_template_directory_uri(); ?>/assets/images/green-dot.svg" alt="Quote"> Chief Operating Officer</p>
 
             <a href="#"  >
                 <img src="<?php echo get_template_directory_uri(); ?>/assets/images/in.svg" alt="Arrow Right">
             </a>
         </div>
       
        </div>
     </diuv>
 </li>
 </ul>


    </div>
</diov>



	</div>
</section>
