...
(function() {
	...
	const initHeader = function() {
		...
		// Store original logo src (captured on page load) for non-sticky state
		const originalLogoSrc = logoImg ? logoImg.getAttribute('src') : '';
		// Prefer data attributes from header (work with ACF/custom logo URLs)
		const dataLogoBlack = logoImg ? logoImg.getAttribute('data-logo-black') : '';
		const dataLogoWhite = logoImg ? logoImg.getAttribute('data-logo-white') : '';

		// Fallback: determine logo path by string replace if data attributes missing
		...
		const updateStickyHeaderElements = function(isSticky) {
			// Update logo: use data-logo-black / original (white) so it works with ACF/custom URLs
			if (logoImg) {
				const newLogoSrc = isSticky
					? (dataLogoBlack || getLogoPath(true))
					: (originalLogoSrc || dataLogoWhite);
