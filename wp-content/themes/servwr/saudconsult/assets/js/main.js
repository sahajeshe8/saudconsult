...
(function() {
	const initPageTabs = function() {
		...
		const activeTabItem = tabsContainer.querySelector('.page_tabs_item.active');
		if (activeTabItem) {
...
(function() {
	var timelineI18n = typeof tasheelI18n !== 'undefined' ? tasheelI18n : {};
	var timelineLoadMoreText = timelineI18n.loadMore || 'Load More';
	var timelineLoadingText = timelineI18n.loading || 'Loading...';
	...
	const initTimelineAnimations = function() {
		...
		const loadMoreItems = function() {
			...
			if (loadMoreBtn) {
				...
				loadMoreBtn.textContent = timelineLoadingText;
			}
			...
			setTimeout(function() {
				...
				if (loadMoreBtn) {
					...
					loadMoreBtn.textContent = timelineLoadMoreText;
				}
			}, 800);
		};
...
(function() {
	var openingsI18n = typeof tasheelI18n !== 'undefined' ? tasheelI18n : {};
	var openingsLoadMoreText = openingsI18n.loadMore || 'Load more';
	var openingsLoadingText = openingsI18n.loading || 'Loading...';
	...
	const initLatestOpeningsLoadMore = function() {
		...
		const loadMoreItems = function() {
			...
			isLoading = true;
			loadMoreBtn.classList.add('loading');
			loadMoreBtn.textContent = openingsLoadingText;
			...
			setTimeout(function() {
				...
				isLoading = false;
				loadMoreBtn.classList.remove('loading');
				loadMoreBtn.textContent = openingsLoadMoreText;
			}, 500);
		};
...
(function() {
	var newsI18n = typeof tasheelI18n !== 'undefined' ? tasheelI18n : {};
	var newsLoadMoreText = newsI18n.loadMore || 'Load More';
	var newsLoadingText = newsI18n.loading || 'Loading...';
	...
	const initNewsPageLoadMore = function() {
		...
		const loadMoreItems = function() {
			...
			isLoading = true;
			loadMoreBtn.classList.add('loading');
			loadMoreBtn.querySelector('span').textContent = newsLoadingText;
			...
			setTimeout(function() {
				...
				isLoading = false;
				loadMoreBtn.classList.remove('loading');
				loadMoreBtn.querySelector('span').textContent = newsLoadMoreText;
			}, 500);
		};
...
(function() {
	var eventsI18n = typeof tasheelI18n !== 'undefined' ? tasheelI18n : {};
	var eventsLoadMoreText = eventsI18n.loadMore || 'Load More';
	var eventsLoadingText = eventsI18n.loading || 'Loading...';
	...
	const initEventsPageLoadMore = function() {
		...
		const loadMoreItems = function() {
			...
			isLoading = true;
			loadMoreBtn.classList.add('loading');
			loadMoreBtn.textContent = eventsLoadingText;
			...
			setTimeout(function() {
				...
				isLoading = false;
				loadMoreBtn.classList.remove('loading');
				loadMoreBtn.textContent = eventsLoadMoreText;
			}, 500);
		};
					...
					'reveal': function(fancybox, slide) {
						...
						if (closeButton) {
							...
							newCloseButton.addEventListener('click', function(e) {
								...
								// Clear any pending popup to prevent opening another popup after close
...
(function() {
	document.addEventListener('DOMContentLoaded', function() {
		...
		function initPopupHandler() {
			...
			// Expose function to clear pending popup (for close button handlers)
			window.clearPendingPopup = function() {
				pendingPopup = null;
			};
			...
			document.addEventListener('fancybox:close', function(e) {
				// Don't open pending popup if close was triggered by close button
				// (close button handlers will have cleared pendingPopup)
				if (!pendingPopup) {
					return;
				}
				...
				breakpoints: {
					0: { // Mobile
						autoplay: {
							delay: 3000,
							disableOnInteraction: false,
							pauseOnMouseEnter: false, // important for mobile
						}
					},
					700: { // Tablet & Desktop
						autoplay: false
...
(function() {
	function initSumoInBlock(block) {
		...
		block.querySelectorAll('select.select-input').forEach(function(select) {
			...
			select.setAttribute('data-sumo-select-initialized', 'true');
			jQuery(select).SumoSelect({ search: true, placeholder: 'Select an item' });
		});
	}
...
jQuery(document).ready(function () {
    MySelectS = jQuery('.project_filter_select').SumoSelect({
        search: true,          // Enable search box
        searchText: 'Search...', // Placeholder text (optional)
        noMatch: 'No matches found' // Text when no result (optional)
    });
