	...
	<div class="engineering_expertise_content_01" data-expertise-container>
		<div class="expertise_image_wrapper">
			<?php if ( ! empty( $active_item ) && isset( $active_item['image'] ) ) : 
				$active_img = $active_item['image'];
				$active_img_mobile = ! empty( $active_item['image_mobile'] ) ? $active_item['image_mobile'] : $active_img;
				$bg_style = 'background: url(' . esc_attr( $active_img ) . ') center center / cover no-repeat;';
			?>
				<div class="expertise_active_image expertise_image_1" role="img" aria-label="<?php echo esc_attr( isset( $active_item['title'] ) ? $active_item['title'] : '' ); ?>"
					data-desktop-bg="<?php echo esc_url( $active_img ); ?>"
					data-mobile-bg="<?php echo esc_url( $active_img_mobile ); ?>"
					style="<?php echo esc_attr( $bg_style ); ?>"></div>
				<div class="expertise_active_image expertise_image_2" role="img" aria-label="<?php echo esc_attr( isset( $active_item['title'] ) ? $active_item['title'] : '' ); ?>"
					data-desktop-bg="<?php echo esc_url( $active_img ); ?>"
					data-mobile-bg="<?php echo esc_url( $active_img_mobile ); ?>"
					style="<?php echo esc_attr( $bg_style ); ?>"></div>
			<?php endif; ?>
		...
		<div class="expertise_thumb_row_content">
			<div class="wrap">
				<div class="expertise_content_wrapper">
					...
					<?php if ( ! empty( $active_item ) && isset( $active_item['description'] ) ) : 
						$description = wp_kses_post( $active_item['description'] );
						// Strip outer <p> tags so we don't nest <p> inside our wrapper
						$description = preg_replace( '/^\s*<p[^>]*>/i', '', $description );
						$description = preg_replace( '/<\/p>\s*$/i', '', $description );
						?>
						<div class="expertise_active_description"><p><?php echo $description; ?></p></div>
					<?php endif; ?>
			...
			<?php if ( ! empty( $content_items ) && count( $content_items ) > 1 ) : ?>
