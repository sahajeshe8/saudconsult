<?php
/**
 * Leadership Section for Home – data from Company Options (shared with Leadership/Our Team pages).
 * Same design as Leadership.php: desktop list + mobile swipers, green diamond icon.
 * WPML: Options are translated per language. Section is hidden if no data.
 *
 * @package tasheel
 */

// Company Options first; fall back to taxonomy (leadership-team, executive-team) when empty.
$visionary = function_exists( 'tasheel_get_global_visionary_leadership' ) ? tasheel_get_global_visionary_leadership() : array( 'label' => '', 'title' => '', 'title_span' => '', 'leadership_members' => array() );
$executive = function_exists( 'tasheel_get_global_executive_team' ) ? tasheel_get_global_executive_team() : array( 'title' => '', 'title_span' => '', 'team_members' => array() );

if ( empty( $visionary['leadership_members'] ) && function_exists( 'tasheel_get_section_members' ) ) {
	$visionary['leadership_members'] = tasheel_get_section_members( array( 'members_source' => 'all', 'members_manual' => array() ), 'leadership-team' );
}
if ( empty( $visionary['label'] ) ) {
	$visionary['label'] = esc_html__( 'Our Leadership Team', 'tasheel' );
}
if ( empty( $visionary['title'] ) ) {
	$visionary['title'] = esc_html__( 'Our Visionary', 'tasheel' );
}
if ( empty( $visionary['title_span'] ) ) {
	$visionary['title_span'] = esc_html__( 'Leadership', 'tasheel' );
}
if ( empty( $executive['team_members'] ) && function_exists( 'tasheel_get_section_members' ) ) {
	$executive['team_members'] = tasheel_get_section_members( array( 'members_source' => 'all', 'members_manual' => array() ), 'executive-team' );
}
if ( empty( $executive['title'] ) ) {
	$executive['title'] = esc_html__( 'Our Executive', 'tasheel' );
}
if ( empty( $executive['title_span'] ) ) {
	$executive['title_span'] = esc_html__( 'Team', 'tasheel' );
}

$has_visionary = ! empty( $visionary['leadership_members'] ) || ! empty( $visionary['title'] ) || ! empty( $visionary['title_span'] ) || ! empty( $visionary['label'] );
...
<section class="leadership_section pt_100" style="background: url('<?php echo esc_url( get_template_directory_uri() ); ?>/assets/images/leadershiop_bg_01.svg') no-repeat top left; background-size: 35%;">
	<div class="wrap">
		<div class="leadership_section_inner_content pb_50">
			...
			<ul class="leadership_section_inner_left_list leadership-desktop-list">
				...
				<li data-aos="fade-up" data-aos-duration="800" data-aos-delay="0">
					...
					<span class="lable_text green_text">
						<img src="<?php echo esc_url( $dot_svg ); ?>" alt="">
			...
			<!-- Mobile View: Visionary Swiper -->
			<?php if ( ! empty( $visionary['leadership_members'] ) ) : ?>
			<div class="leadership-mobile-slider">
				<div class="leadership-mobile-header">
					<?php if ( ! empty( $visionary['label'] ) ) : ?><span class="lable_text green_text"><img src="<?php echo esc_url( $dot_svg ); ?>" alt=""><?php echo esc_html( $visionary['label'] ); ?></span><?php endif; ?>
			...
			<div class="leadership_section_inner_right" data-aos="fade-up" data-aos-duration="800" data-aos-delay="300">
				...
				<h4 class="h4_title_35 pb_20"><?php echo esc_html( $executive['title'] ?? '' ); ?> <?php if ( ! empty( $executive['title_span'] ) ) : ?><span><?php echo esc_html( $executive['title_span'] ); ?></span><?php endif; ?></h4>
				<?php endif; ?>

				<!-- Desktop/Tablet View: Executive Team -->
				<ul class="leadership_section_inner_left_list leadership-desktop-list">
					<?php $delay = 400;
