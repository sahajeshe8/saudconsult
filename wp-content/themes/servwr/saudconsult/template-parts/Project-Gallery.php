									...
									<div class="swiper-slide">
										<div class="project_gallery_slide">
											<?php if ( $is_video && $video_url ) :
												// Fancybox 5 auto-detects YouTube/Vimeo and HTML5 video from href.
												// For direct MP4/WebM, use data-type="html5video" so it renders correctly.
												$is_youtube = ( strpos( $video_url, 'youtube.com' ) !== false || strpos( $video_url, 'youtu.be' ) !== false );
												$is_vimeo   = ( strpos( $video_url, 'vimeo.com' ) !== false );
												$is_direct  = ! $is_youtube && ! $is_vimeo;
												$fb_attrs   = 'class="project_gallery_image_link" data-fancybox="project-gallery" data-caption="' . esc_attr( $image_alt ) . '"';
												if ( $is_direct ) {
													$ext = strtolower( pathinfo( parse_url( $video_url, PHP_URL_PATH ), PATHINFO_EXTENSION ) );
													$fb_attrs .= ' data-type="html5video"';
													if ( $ext === 'webm' ) {
														$fb_attrs .= ' data-html5video-format="video/webm"';
													} else {
														$fb_attrs .= ' data-html5video-format="video/mp4"';
													}
												}
											?>
												<a href="<?php echo esc_url( $video_url ); ?>" <?php echo $fb_attrs; ?>>
