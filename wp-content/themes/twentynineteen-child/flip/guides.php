					...
					<?php  while ( $the_query->have_posts() ){
						...
						<figure>
							 <picture>
								 ...
								 $image_webp = wp_get_attachment_image_url($webp_image['ID'],'webp_image');
								// print_r ($image_webp);
								if( !empty( $webp_image ) ): ?>
									...
									<?php */ endif; ?>
										...
										$image_url = wp_get_attachment_image_url($jpg_image['ID'],'webp_image');										
										if( !empty( $jpg_image ) ): ?>
											...
											<?php else: ?>
												...
												<?php 
												$jpg_image = get_field('post_image');												
												$imagall = wp_get_attachment_image($jpg_image['ID'],'webp_image');
												// $image_srcset = wp_get_attachment_image_srcset($jpg_image['ID'],'webp_image');
												// $image_url = wp_get_attachment_image_url($jpg_image['ID'],array(500,500));
												// $image_src = wp_get_attachment_image_src($jpg_image['ID'],'webp_image');
												if( !empty( $jpg_image ) ): ?>
													<?php  echo $imagall; ?>													
												<?php else: ?>
								...
								<div class="content-sidebar-inner">
									...
									if ( $the_query->have_posts() ){ ?>
										...
										<div class="content-block-area">
											<h2 class="h4 mobile-only">Latest Guides</h2>
																			...
																			<?php foreach( $latest_business_guides as $post ): 
																				...
																				<li>
																					<figure>
																						 <picture>
																							<?php $webp_image = get_field('webp_image');
																							$webp_image = wp_get_attachment_image_src($webp_image['ID'], array(100,100));
																							if( !empty( $webp_image ) ): ?>
																								...
																								<?php */ endif; ?>
																									<?php 
																									$jpg_image = get_field('post_image');
																									$image_url = wp_get_attachment_image_src($jpg_image['ID'], array(100,100));
																									if( !empty( $jpg_image ) ): ?>
																										...
																										<?php else: ?>
																											...
																											<?php 
																											$jpg_image = get_field('post_image');
																											$imagall = wp_get_attachment_image($jpg_image['ID'],array(100,100));
																											if( !empty( $jpg_image ) ): ?>
																											<?php echo $imagall; ?>
