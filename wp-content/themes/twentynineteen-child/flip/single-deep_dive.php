...
<main>
	...
	<section class="single-top-image-sec single-top-image-sec-2">
		<div class="container">
			<div class="single-top-image-inner">
				<div class="single-top-image-heading">
				<?php if( !empty( get_field('listing_image') ) ){ ?>
						<figure>
						<picture>
							<?php 
							$webp_image = get_field('listing_imagewebp');
							if( !empty( $webp_image ) ): ?>
								<source srcset="<?php echo esc_url($webp_image['url']); ?>" alt="<?php echo esc_attr($webp_image['alt']); ?>" type="image/webp">
								<?php endif; ?>

								<?php 
								$jpg_image = get_field('listing_image');
								if( !empty( $jpg_image ) ): ?>
	...
	<section class="single-content-widget-sec single-content-widget-sec-2">
	<div class="container">
		<div class="single-content-widget-inner">
			...
			if( $latest_business_guides ): ?>
				...
				<div class="single-content-center">
					<?php if( have_rows('deep_dives_content') ): ?>
						<?php while( have_rows('deep_dives_content') ): the_row();  ?>      
						         
							<?php if(get_sub_field('title_format')) { echo '<'.get_sub_field('title_format') .' class="'. get_sub_field('dd_class'). '" id="'.get_sub_field('dd_id').'" >'; }else { echo'<h4>'; }?>
							<?php the_sub_field('dd_title'); ?>
					<?php if(get_sub_field('title_format')) { echo '</'.get_sub_field('title_format') .'>'; }else { echo'</h4>'; }?>
							<?php the_sub_field('dd_content'); ?>    
						<?php endwhile; ?>
					<?php endif; ?>
				</div>
				<?php
				...
				<div class="single-widget-right" id="fixed-outer">
					<div id="fixed-area">
